...
test('ACTION_VERSION uses a stable release tag format', () => {
  assert.match(ACTION_VERSION, /^v[0-9]+\.[0-9]+\.[0-9]+$/)
})
...
test('run creates a rendered comment with pull request fallback metadata', async () => {
  ...
  assert.equal(core.calls.info[0], `comment-action version: ${ACTION_VERSION}`)
