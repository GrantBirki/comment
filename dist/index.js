...
var NodeResolveLoader = /*#__PURE__*/function (_Loader2) {
  ...
  _proto2.getSource = function getSource(name) {
    ...
    var fullpath;
    try {
      fullpath = __WEBPACK_EXTERNAL_createRequire(import.meta.url).resolve(name);
    } catch (e) {
      return null;
    }
    this.pathsToNames[fullpath] = name;
...
// UNUSED EXPORTS: LocalActionsCore, REACTION_TYPES, SafeTemplateLoader, addReactions, appendSeparatorTo, applyReactions, createComment, createGithubClient, getAuthenticatedUser, getCommentReactionsForUser, getInputs, parseVars, renderComment, replaceReactions, resolveBody, resolveIssueNumber, resolveRepository, sanitizeInputs, truncateBody, updateExistingComment, validReactions
...
function issueCommand(command, message, properties = {}) {
    ...
    const propertyText = propertyEntries.length === 0
        ? ''
        : ` ${propertyEntries
            .map(([key, value]) => `${key}=${escapeCommandProperty(value)}`)
            .join(',')}`;
    process.stdout.write(`::${command}${propertyText}::${escapeCommandData(message)}${external_node_os_namespaceObject.EOL}`);
}
function inputEnvironmentName(name) {
    return `INPUT_${name.replace(/ /g, '_').toUpperCase()}`;
}
class LocalActionsCore {
    env;
    ...
    setOutput(name, value) {
        ...
        if (outputPath) {
            let delimiter = `comment_${(0,external_node_crypto_namespaceObject.randomUUID)()}`;
            while (output.includes(delimiter)) {
                delimiter = `comment_${(0,external_node_crypto_namespaceObject.randomUUID)()}`;
            }
            external_node_fs_default().appendFileSync(outputPath, `${name}<<${delimiter}${external_node_os_namespaceObject.EOL}${output}${external_node_os_namespaceObject.EOL}${delimiter}${external_node_os_namespaceObject.EOL}`);
            return;
        }
...
function readGithubPayload(env) {
    const eventPath = env.GITHUB_EVENT_PATH;
    if (!eventPath || !external_node_fs_default().existsSync(eventPath)) {
        return {};
    }
    try {
        return JSON.parse(external_node_fs_default().readFileSync(eventPath, 'utf8'));
    }
    catch {
        return {};
    }
}
function createGithubClient(env = process.env) {
    return {
        context: {
...
class LocalOctokit {
    token;
    apiUrl;
    constructor(token, env = process.env) {
        this.token = token;
        this.apiUrl = (env.GITHUB_API_URL || 'https://api.github.com').replace(/\/+$/, '');
    }
    paginate = {
        iterator: async function* (endpoint, options) {
    ...
    rest = {
        issues: {
            createComment: async (options) => {
                return this.request({
                    method: 'POST',
                    path: `/repos/${encodePath(options.owner)}/${encodePath(options.repo)}/issues/${encodePath(options.issue_number)}/comments`,
            ...
            getComment: async (options) => {
                return this.request({
                    method: 'GET',
                    path: `/repos/${encodePath(options.owner)}/${encodePath(options.repo)}/issues/comments/${encodePath(options.comment_id)}`
                });
            },
        ...
        reactions: {
            createForIssueComment: async (options) => {
                return this.request({
                    method: 'POST',
                    path: `/repos/${encodePath(options.owner)}/${encodePath(options.repo)}/issues/comments/${encodePath(options.comment_id)}/reactions`,
                    body: { content: options.content }
                });
            },
            deleteForIssueComment: async (options) => {
                return this.request({
                    method: 'DELETE',
                    path: `/repos/${encodePath(options.owner)}/${encodePath(options.repo)}/issues/comments/${encodePath(options.comment_id)}/reactions/${encodePath(options.reaction_id)}`
                });
            },
            listForIssueComment: async (options) => {
                return this.request({
                    method: 'GET',
                    path: `/repos/${encodePath(options.owner)}/${encodePath(options.repo)}/issues/comments/${encodePath(options.comment_id)}/reactions`,
                    query: {
    ...
    async request({ method, path: requestPath, query = {}, body }) {
...
function getInputs(actionsCore = defaultActionsCore) {
...
function prepareVarsLines(vars) {
...
function parseVarsScalar(value) {
    if (value === 'true') {
        return true;
    }
    if (value === 'false') {
...
function parseVarsBlock(lines, index, indent) {
...
function validReactions(reactions, actionsCore = defaultActionsCore) {
    return [
        ...new Set(reactions
            .split(/[\n,]+/)
            .map(item => item.trim())
...
async function addReactionSet(octokit, repo, commentId, reactionSet, actionsCore = defaultActionsCore) {
...
async function addReactions(octokit, repo, commentId, reactions, actionsCore = defaultActionsCore) {
...
async function removeReactions(octokit, repo, commentId, reactions, actionsCore = defaultActionsCore) {
...
async function replaceReactions(octokit, repo, commentId, reactionSet, actionsCore = defaultActionsCore) {
...
async function applyReactions(octokit, repo, commentId, reactions, reactionsEditMode, actionsCore = defaultActionsCore) {
    const reactionSet = validReactions(reactions, actionsCore);
    if (reactionSet.length === 0) {
        actionsCore.setFailed(`No valid reactions are contained in '${reactions}'.`);
        return false;
...
function truncateBody(body, actionsCore = defaultActionsCore) {
...
async function run(options = {}) {
    ...
    try {
        ...
        actionsCore.debug(`Inputs: ${(0,external_node_util_namespaceObject.inspect)(sanitizeInputs(inputs))}`);
        ...
        actionsCore.debug((0,external_node_util_namespaceObject.inspect)(error));
